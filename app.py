# to dos
# add full images
# reduce displayed columns (get from Andre)
# Replace Phenotype with "Disease Group"
# Remove date from hover_info
# Cache Images
# cell type abundance
# Load Images to Box
...
def plot_from_data(df, label = "phenotypes", draw_centroid = True, draw_ellipse = True, pca_loadings = None):
    ...
    if df[label].dtype == "object":
        ...
        for target, group in df.groupby(label):
            ...
            customdata = customdata.replace({"nan": "NA", "": "NA"}, regex = False)
